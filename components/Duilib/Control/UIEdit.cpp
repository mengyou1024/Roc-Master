...
namespace DuiLib {
    ...
    void CEditUI::SetPos(RECT rc) {
        ...
        if (m_pWindow != NULL) {
            ...
            ::SetWindowPos(m_pWindow->GetHWND(), NULL, rcPos.left, rcPos.top, rcPos.right - rcPos.left, rcPos.bottom - rcPos.top,
    ...
    void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
        ...
        else if (_tcscmp(pstrName, _T("textext")) == 0)
            SetTextExt(pstrValue);
    ...
    void CEditUI::PaintText(HDC hDC) {
        ...
        if (IsEnabled()) {
            CRenderEngine::DrawText(hDC, m_pManager, rc, sText, mCurTextColor, m_iFont, DT_SINGLELINE | m_uTextStyle);
        } else {
